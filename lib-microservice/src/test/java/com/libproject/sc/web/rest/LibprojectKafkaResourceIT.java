...
import com.libproject.sc.config.KafkaProperties;
...
class LibprojectKafkaResourceIT {
    ...
    void setup() {
        ...
        LibprojectKafkaResource kafkaResource = new LibprojectKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/libproject-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/libproject-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
